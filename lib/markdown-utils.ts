 ...
 */
export function parseJobTitleLine(text: string): { position: string; company: string; date: string } {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === 'heading_open') {
      ...
      if (nextToken && nextToken.type === 'inline') {
        ...
        } else if (currentSection && (token.tag === 'h3' || token.tag === 'h4')) {
      ...
      if (nextToken && nextToken.type === 'inline') {
        ...
        if (isContactInfo(paragraphText)) {
          ...
          if (isJobTitleLine(paragraphText)) {
            const { position, company, date } = parseJobTitleLine(paragraphText);
            currentSection.content.push({
              type: 'job-title',
